...
namespace MOBAManager.MatchResolution
{
    partial class MatchAI
    {
        ...
        /// <summary>
        /// Resolves the various after-match effects handled by the Match AI, including hero-specific experience.
        /// </summary>
        ...
        private List<Tuple<Player, Hero>> FinalizeLineup(int team)
        {
            ...
            if (team == 1)
            {
                curTeam = team1.GetTeammates();
                curPicks = team1Picks;
            }
            else
            {
                curTeam = team2.GetTeammates();
                curPicks = team2Picks;
            }
        ...
        private double CalculateLineupSkill(int team, List<Tuple<Player, Hero>> lineup)
        {
            ...
            if (team == 1)
            {
                finalValue += team1.GetTeamworkSkill();
            }
            else
            {
                finalValue += team2.GetTeamworkSkill();
                oppPicks = team1Picks;
            }
